...
class NodeUI {
    ...
    disableNode(id, options) {
        ...
        // Determine which outgoing targets to bypass to. For conditional nodes,
        // when unchecked/disabled we prefer only the edge labeled No.
        // Fallback to all outgoing edges when no label is available.
        let outs = this._outgoingOf(id);
        if (Array.isArray(n.connections)) {
            const noEdges = n.connections.filter(c => c && typeof c.text === 'string' && c.text.trim().toLowerCase() === 'no');
            if (noEdges.length > 0) {
                outs = Array.from(new Set(noEdges.map(c => c.to)));
            }
        }
